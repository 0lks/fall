...
public class GameControl : MonoBehaviour {
    ...
    */
    ...
    void Awake () {
        hexPrefab = (GameObject) Resources.Load("1_PREFABS/2_Other/Hex/Hex");
        wolf = (GameObject) Resources.Load("1_PREFABS/6_Characters/WolfPREFAB");
        xRay = (Shader)Resources.Load("2_MATERIALS_SHADERS/XRay/XRay");
        noXRay = (Shader)Resources.Load("2_MATERIALS_SHADERS/XRay/NOXRay");
    ...
    // Called by StartGame(), resets the game to its original state
    {
        ...
        if (player.spawnPoint != null && map.HexExists(player.spawnPoint))
        {
            player.NewCurrentPosition(map.GetHex(player.spawnPoint));
        }
